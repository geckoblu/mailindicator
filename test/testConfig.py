...
import tempfile
import unittest
from testconstants import TESTCONFIG
...
class TestConfig(unittest.TestCase):
    ...
    def load2(self):
        config.use_proxy = True
        config.http_proxy = 'http://127.0.0.1:6060'
        config.https_proxy = 'http://127.0.0.1:6060'
    
        config.mailboxes = []
        mailbox  = Mailbox(Mailbox.GMAILFEED, 'GMAIL', 300, username='alessio.piccoli', userpassword='Echo09.box2')
        config.mailboxes.append(mailbox)
        
        mailbox = Mailbox(Mailbox.LOCALMBOX, 'System', 60, mboxpath='/var/spool/mail/alessio')
        config.mailboxes.append(mailbox)
        
        mailbox = Mailbox(Mailbox.IMAPSTARTTLS, 'Work', 60, username='b02183', userpassword='Leonida789', host='mercurio.it.wuerth.com')
        config.mailboxes.append(mailbox)
